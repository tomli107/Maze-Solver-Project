...
// End of "DO NOT CHANGE" Block
  ...
  for(int i=0;i<notprimes[size_index-1];i++){
    if(temp[i].key != NULL) {
      int p = hash1(temp[i].key->getUniqId());
      int inc = hash2(temp[i].key->getUniqId());
      while(table[p].key != NULL){
        p = (p+inc) % size;
      }
      table[p]=temp[i];
    }
...
bool DoubleHashDict::find(MazeState *key, MazeState *&pred) {
  ...
  int p = hash1(key->getUniqId());
  int inc = hash2(key->getUniqId());

  while(table[p].key != NULL){
    if(table[p].key->getUniqId() == key->getUniqId()){
      pred = table[p].data;
      numProbes++;
      record_stats(numProbes);
      return true;
    }
    p = (p+inc) % size;
    numProbes++;
